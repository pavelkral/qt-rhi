...
float shadowCalculationVulkan(vec3 normal, vec3 fragPos)
{
    ...
    // !!! u Vulkan/D3D už není potřeba *0.5 + 0.5
    // protože clipSpaceCorrMatrix už transformovalo souřadnice do [0..1]
    //vec3 projCoords = fragPosLightSpace.xyz / fragPosLightSpace.w;
    ...
    if (projCoords.z < 0.0) {
        return 0.0; // před near plane světla
    }

    float currentDepth = projCoords.z;

    // bias proti shadow acne
