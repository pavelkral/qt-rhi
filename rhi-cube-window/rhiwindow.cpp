...
void RhiWindow::render()
{
    ...
    // Bezpečnostní kontrola (neměla by nastat po Success, ale ať to nespadne)
    if (!m_sc->currentFrameRenderTarget()) {
        qWarning("currentFrameRenderTarget() is null, skipping frame.");
...
// 8 unikátních vrcholů (pozice + UV)
...
// 36 indexů (6 stěn × 2 trojúhelníky × 3 indexy)
static const quint16 cubeIndices[] = {
    0,  1,  2,   0,  2,  3,      // front
    4,  5,  6,   4,  6,  7,      // back
    8,  9, 10,   8, 10, 11,      // left
...
void createInstances() {
    ...
    std::uniform_real_distribution<float> depthDist(-13.0f, -8.0f);
    std::uniform_real_distribution<float> axisDist(0.0f, 1.0f);
    std::uniform_real_distribution<float> speedDist(0.5f, 2.0f);

    for (int i = 0; i < 6; ++i) {
        CubeInstance ci;
        ci.position = QVector3D(posDist(rng), posDist(rng), depthDist(rng));
       // ci.rotationAxis = QVector3D(axisDist(rng), axisDist(rng), axisDist(rng)).normalized();
        ci.rotationAxis = QVector3D(0,1,0);
        ci.rotationSpeed= speedDist(rng);
        cubes.append(ci);
    }
}
...
void HelloWindow::loadTexture(const QSize &, QRhiResourceUpdateBatch *u)
{
    ...
    QImage image(":/floor.png");          // <- vlož svou texturu do resource pod tímto aliasem
    if (image.isNull()) {
...
void HelloWindow::customInit()
{
    ...
    // --- VBO (8 vrcholů) ---
    m_vbuf.reset(m_rhi->newBuffer(QRhiBuffer::Immutable, QRhiBuffer::VertexBuffer, sizeof(cubeVertices)));
    m_vbuf->create();
    m_initialUpdates->uploadStaticBuffer(m_vbuf.get(), cubeVertices);

    // --- IBO (36 indexů) ---
    m_ibuf.reset(m_rhi->newBuffer(QRhiBuffer::Immutable, QRhiBuffer::IndexBuffer, sizeof(cubeIndices)));
    m_ibuf->create();
    m_initialUpdates->uploadStaticBuffer(m_ibuf.get(), cubeIndices);

    // --- UBO (MVP matice) ---
    static const quint32 UBUF_SIZE = 64;
    m_ubuf.reset(m_rhi->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer,6* sizeof(UniformBlock)));
    m_ubuf->create();

    // --- Textura (jednorázové načtení z resource) ---
    loadTexture(QSize(), m_initialUpdates); // používáme ji pro krychli
    ...
    // --- SRB: binding0 = UBO (VS), binding1 = texture+sampler (FS) ---
    ...
    // --- Pipeline pro texturovanou krychli ---
    ...
    inputLayout.setBindings({ { 5 * sizeof(float) } });         // pos3 + uv2
    ...
    //rub->uploadStaticBuffer(m_vbuf, cubeVertices);
   // rub->uploadStaticBuffer(m_ibuf, cubeIndices);
    // vytvoření několika krychlí
    createInstances();

   // m_initialUpdates->append(rub);
}
...
void HelloWindow::customRender()
{
    ...
    mvp.rotate(m_rotation, 0.0f, 1.0f, 0.0f);
  ...
  //cb->drawIndexed(36);
    ...
    QMatrix4x4 proj;
    proj.perspective(45.0f, float(m_sc->surfacePixelSize().width()) / m_sc->surfacePixelSize().height(), 0.1f, 100.0f);

    QMatrix4x4 view;
    view.lookAt(QVector3D(0, 0, 5), QVector3D(0, 0, 0), QVector3D(0, 1, 0));

    static float elapsed = 0.0f;
    elapsed += 0.016f; // ~60 FPS simulace

    size_t ubSize = sizeof(UniformBlock);

    for (CubeInstance &ci : cubes) {
        ci.modelMatrix.setToIdentity();
        ci.modelMatrix.translate(ci.position);
        ci.modelMatrix.rotate(elapsed * ci.rotationSpeed * 50.0f, ci.rotationAxis);

        UniformBlock ub;
        ub.mvp = proj * view * ci.modelMatrix;

        // aktualizace bufferu vždy od začátku
        resourceUpdates->updateDynamicBuffer(m_ubuf.get(), 0, sizeof(UniformBlock), &ub);
